...
function build(captions, options) {
  ...
  for (var i = 0; i < captions.length; i++) {
    ...
    if (caption.type == "meta") {
      ...
      if (
        caption.data != null &&
        typeof caption.data === `object` &&
        `PlayResX` in caption.data &&
        `PlayResY` in caption.data
      ) {
        playResX = Number(caption.data.PlayResX);
        playResY = Number(caption.data.PlayResY);
      }
    ...
    if (typeof caption.type === "undefined" || caption.type == "caption") {
      ...
      const match = posRegex.exec(caption.content);
      if (match != null && playResX != null && playResY != null) {
        const [x, y] = match[1].split(`,`).map(Number);
        const xPerc = `${(100 * x) / playResX}%`;
        const yPerc = `${(100 * y) / playResY}%`;
        content += ` position:${xPerc} line:${yPerc}`;
      }
      content += eol;
